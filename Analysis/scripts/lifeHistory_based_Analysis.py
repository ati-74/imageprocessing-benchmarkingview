...
def plot(df1 , df2,df3 , df4 , df5, dataset , plot_title, min_val , max_val ,num_bins,Tools_name,feature):
    ...
    if dataset !="Mono Culture" or feature not in ["AverageVelocity","AverageLength"]:
        ax.bar(b_bins[:-1]+width, b_heights, width=width, facecolor='black',label=Tools_name[1])
    ax.bar(c_bins[:-1]+2*width, c_heights, width=width, facecolor='green',label=Tools_name[2])
    ax.bar(d_bins[:-1]+3*width, d_heights, width=width, facecolor='yellow',label=Tools_name[3])
    ax.bar(e_bins[:-1]+4*width, e_heights, width=width, facecolor='blue',label=Tools_name[4])    
    plt.grid(False, axis='x')

    #bins
    bins_str = []
    for i in range(len(a_bins)-1):
        bins_str.append(str(np.round(a_bins[i],2))+"-"+str(np.round(a_bins[i+1],2)))
    plt.xticks(ticks=a_bins[:len(a_bins)-1], labels=bins_str , rotation = 90 , fontsize=6 )
...
def life_history_based_distribution(features ,end_of_file_name,Tools_name,datasets,main_directories,plot_titles):
    num_bins = 30
    for feature in features:
        for dataset in datasets:
            CP_csv_file = main_directories['CP_directory']+dataset+"/post-processing/results/"+Tools_name[0]+'_'+end_of_file_name+".csv"
            DeLTA_csv_file = main_directories['DeLTA_directory']+dataset+"/post-processing/results/"+Tools_name[1]+'_'+end_of_file_name+".csv"
            FAST_csv_file = main_directories['FAST_directory']+dataset+"/post-processing/results/"+Tools_name[2]+'_'+end_of_file_name+".csv"
            Oufti_csv_file = main_directories['Oufti_directory']+dataset+"/post-processing/results/"+Tools_name[3]+'_'+end_of_file_name+".csv"
            SuperSegger_csv_file = main_directories['SuperSegger_directory']+dataset+"/post-processing/results/"+Tools_name[4]+'_'+end_of_file_name+".csv"
            #read csv file
            ...
            if dataset =="Mono Culture" and (feature in ["AverageVelocity","AverageLength"]):
                #draw plot
                max_val = max(df_cp.values.max(),df_fast.values.max(),df_oufti.values.max(),df_supersegger.values.max())
                min_val = min(df_cp.values.min(),df_fast.values.min(),df_oufti.values.min(),df_supersegger.values.min())
                plot(df_cp , df_delta,df_fast , df_oufti , df_supersegger, dataset , plot_titles[feature], min_val , max_val ,num_bins,Tools_name,feature)

            else:    
                #draw plot
                max_val = max(df_cp.values.max(),df_delta.values.max(),df_fast.values.max(),df_oufti.values.max(),df_supersegger.values.max())
                min_val = min(df_cp.values.min(),df_delta.values.min(),df_fast.values.min(),df_oufti.values.min(),df_supersegger.values.min())
                plot(df_cp , df_delta,df_fast , df_oufti , df_supersegger, dataset , plot_titles[feature], min_val , max_val ,num_bins,Tools_name,feature)



def lineage_based_distribution(features ,end_of_file_name,Tools_name,datasets,main_directories,plot_titles):
    num_bins = 30
    for feature in features:
        for dataset in datasets:
            CP_csv_file = main_directories['CP_directory']+dataset+"/post-processing/results/"+Tools_name[0]+'_'+end_of_file_name+".csv"
...
def timestep_based_distribution(features ,end_of_file_name,Tools_name,datasets,main_directories,plot_titles):
    for feature in features:
        for dataset in datasets:
            CP_csv_file = main_directories['CP_directory']+dataset+"/post-processing/results/"+Tools_name[0]+'_'+end_of_file_name+".csv"
            DeLTA_csv_file = main_directories['DeLTA_directory']+dataset+"/post-processing/results/"+Tools_name[1]+'_'+end_of_file_name+".csv"
            ...
            df_supersegger = pd.read_csv(SuperSegger_csv_file, usecols=[str(feature)])
            df_supersegger = df_supersegger.rename(columns={str(feature): 'SuperSegger'})
            #concatinate columns
            df = pd.concat([df_cp, df_delta , df_fast, df_oufti, df_supersegger], axis=1)
            df.index = np.arange(1, len(df)+1)
            plot = df.plot(kind="bar" , color = ['red','black', 'green' , 'yellow' , 'blue'])
...
def bac_feature_distribution(features ,end_of_file_name,Tools_name,datasets,main_directories,plot_titles):
    num_bins = 30
    for feature in features:
        for dataset in datasets:
            CP_csv_file = main_directories['CP_directory']+dataset+"/post-processing/results/"+Tools_name[0]+'_'+end_of_file_name+".csv"
...
if __name__=="__main__":
    ...
    #datasets
    datasets = ["Mono Culture","Schnitzcells sample images set","SuperSegger sample images set"]

    #features
    features={
    'features_lifehistory_based' : ["birthLength","AverageLength","AverageVelocity","LifeHistory","GrowthRate"],
    ...
    #titles    
    plot_titles={
    'features_lifehistory_based' : {"birthLength":"birth length distribution",
    ...
    'feature_lineage_based' : {"NumberOfDivision":"detected cell division in cells lineage"},
    'feature_bac_feature' : {"Orientation":"orientation of bacteria in each time step "},
    'feature_timeStep_based' : {"NumberOfCells":"number of cells in each time step"}
    }    
    ...
    #rimestep_based
    timestep_based_distribution(features['feature_timeStep_based'] ,end_of_file_names["feature_timeStep_based"],Tools_name,datasets,main_directories,plot_titles["feature_timeStep_based"])
