...
def ProcessData(input_file, interval_time, growth_rate_method="Average"):
    ...
    dataFrame=dataFrame.loc[(dataFrame["TrackObjects_Label_50"].notnull()) &
                                  (dataFrame["AreaShape_MajorAxisLength"] != 0)].reset_index(drop=True)
    dataFrame = dataFrame.reset_index(drop=True)

    # process the tracking data
    df , life_history_based_analysis = BacteriaAnalysis(dataFrame, interval_time, growth_rate_method)
    lineage_based_analysis_results = lineage_based_analysis(df)
    Num_cells_in_each_timeStep_results = Num_cells_in_each_timeStep(df)

    # write to csv
    path_lifehistory = "../results/CP_life_history_based_analysis"
    path_lineage = "../results/CP_lineage_based_analysis"
    path_bacteria_based_features = "../results/CP_bacteria_based_features"
    ...
    Num_cells_in_each_timeStep_results.to_csv(path_Num_cells_in_each_timeStep + ".csv", index=False)


if __name__ == "__main__":
    input_file = (
        "../../4. CP outputs/MyExpt_IdentifySecondaryObjects.csv"
    )
    interval_time = 1.5
